...
fn lower_optionals(
    optionals: &[crate::sparql::ast::OptionalPattern],
    schema: &Schema,
) -> Result<Vec<OptionalGroup>, DarqError> {
    let mut result = Vec::new();
    for opt in optionals {
        let ggp = GroupGraphPattern {
            patterns: opt.patterns.clone(),
            filters: vec![],
            optionals: vec![], subqueries: vec![], unions: vec![],
        };
        ...
        let expr_filters: Vec<FilterExpr> = opt.filters.iter().filter_map(|f| {
            match f {
                Filter::Expression(e) => Some(e.clone()),
                _ => None,
            }
        }).collect();
        result.push(OptionalGroup { patterns, expr_filters });
    }
    Ok(result)
}
...
fn augment_with_type(ggp: &GroupGraphPattern, subject_name: &str, type_iri: &Iri) -> GroupGraphPattern {
    ...
    GroupGraphPattern {
        ...
        subqueries: ggp.subqueries.clone(), unions: ggp.unions.clone(),
    }
}
...
mod tests {
    ...
    fn test_lower_empty_bgp() {
        ...
        let bgp = GroupGraphPattern { patterns: vec![], filters: vec![], optionals: vec![], subqueries: vec![], unions: vec![] };
        let patterns = lower_bgp(&bgp, &schema).unwrap();
